...
function [Ff meanFf maxFf stdFf NoiseParamSub NoiseParamCant t]=MultibondCriticalStretchPeriodicPotential(v,T,GamSub,GamCant,ender,NoiseMult,n,ksub,kcant,timeStep,aTimes,PeriodicAmplitudeOffset)
clc
clear X x MeanFf BondState tFree Ff StretchAtDebonding

m=.000000000000001;%The mass should be very very small
k=ksub;K=kcant;%Cantilever and substrate spring constants
a=.1E-9;%The critical stretch length
kB=1.38E-23;w0=1E10;

gam=GamSub+GamCant;%A convenient damping parameter
ThetaT=GamCant/gam;%Another convenient damping parameter

NoiseParamSub=sqrt(2*GamSub*T*kB/timeStep);%Noise amplitude from fluctuation-dissipation theorem for substrate damping
NoiseParamCant=sqrt(2*GamCant*T*kB/timeStep)%Noise amplitude from fluctuation-dissipation theorem for cantilever damping
NoiseParamIndep=0;%If external noise is independent of damping

xc=1e-10; %critical stretch length
...
NoiseTot=PeriodicAmplitudeOffset+(NoiseMult*normrnd(0,NoiseParamCant,length(t),1)+NoiseMult*normrnd(0,NoiseParamSub,length(t),1)+NoiseMult*normrnd(0,NoiseParamIndep,length(t),1));%The noise amplitude comes from a random distribution with standard deviations determined by the noise parameters
...
%     X(i+1)=X(i)+1/6*(l1+2*l2+2*l3+l4);
    k1=tStep/gam*(NoiseTot(i)+1e-9*sin(2*pi*(x(i,:)-X(i))/xc)-K*X(i)+GamSub*v);
    k2=tStep/gam*(NoiseTot(i)+1e-9*sin(2*pi*(x(i,:)-(X(i)+k1/2))/xc)-K*(X(i)+k1/2)+GamSub*v);
    k3=tStep/gam*(NoiseTot(i)+1e-9*sin(2*pi*(x(i,:)-(X(i)+k2/2))/xc)-K*(X(i)+k2/2)+GamSub*v);
    k4=tStep/gam*(NoiseTot(i)+1e-9*sin(2*pi*(x(i,:)-(X(i)+k3))/xc)-K*(X(i)+k3)+GamSub*v);
    X(i+1)=X(i)+1/6*(k1+2*k2+2*k3+k4);
    
%     k1=tStep/gam*(NoiseTot+1e-9*sin(2*pi*(x(i,:)-X(i))/xc)-gam*dXdt(i)-K*X(i)+GamSub*v);
%     k2=tStep/gam*(NoiseTot+1e-9*sin(2*pi*(-(X(i)+l1/2)+x(i,:))/xc)-gam*(dXdt(i)+k1/2)-K*(X(i)+l1/2)+GamSub*v);
%     k3=tStep/gam*(NoiseTot+1e-9*sin(2*pi*(-(X(i)+l2/2)+x(i,:))/xc)-gam*(dXdt(i)+k2/2)-K*(X(i)+l2/2)+GamSub*v);
%     k4=tStep/gam*(v);
%     X(i+1)=X(i)+1/6*(k1+2*k2+2*k3+k4);
