...
% scale
velocity=1e-4*ones(1,1);%Or, one speed can be tested
...
aTimes=20;%Used to either lengthen or shorten the time to run the simulation
...
for i=1:length(velocity)%Runs for each velocity
    [MeanFf(i) MaxFf(i) StdFf(i) tNotBonded(i) NoiseParamSub(i) NoiseParamCant(i) FirstSlipForce(i)]=MultibondCriticalStretchRungKuttaHarmonicPotential(velocity(i),V2,Temp(i),gammaSub(i),gammaCant(i),Ender(i),noiseMult,n,ksub,kcant,timeStep,aTimes);
end
